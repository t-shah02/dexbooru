...
export const isStrongPassword = (password: string): boolean | string[][] => {
	...
	if (isMinimumLength && hasLowerLetter && hasUpperLetter && hasDigit && hasSpecialChar)
		return true;
...
export const isValidUsername = (username: string): boolean | string[][] => {
	let inLengthRange =
		username.length >= MINIMUM_USERNAME_LENGTH && username.length <= MAXIMUM_USERNAME_LENGTH;
	let hasLetter = false;
